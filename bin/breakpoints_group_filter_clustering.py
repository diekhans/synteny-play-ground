...
class Breakpoint:
    def __init__(self, chrom, start, end, qChrom1, qStart, qChrom2, qEnd, insTarget, queryNs, targetNs):
        self.chrom = chrom
        self.start = start
        self.end = end
    ...
    with open(path) as f:
        for line in f:
            ...
            b = Breakpoint(line[0], int(line[1]), int(line[2]), line[3], int(line[4]), line[5],\
                 int(line[6]), bool(line[7]), float(line[8]), float(line[9]))
            breaks.append(b)
